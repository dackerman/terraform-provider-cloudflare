...
resource "cloudflare_fallback_domain" "example" {
  account_id = "f037e56e89293a057740de681ac9abbe"
  domains    = concat([for domains in [for value in toset(["intranet", "internal", "private", "localdomain", "domain", "lan", "home", "host", "corp", "local", "localhost", "home.arpa", "invalid", "test"]) : { key = value, value = value }] : { content = { suffix = domains.value } }], [{ description = "Example domain", dns_server = ["192.0.2.0", "192.0.2.1"], suffix = "example.com" }])
}`

	diags := ast.Diagnostics{}
	body := ast.ParseIntoSyntaxBody([]byte(tf), "test.tf", diags)
	b2 := ast.Block2AttrsRoot(*body, diags)
	actual := string(hclwrite.Format([]byte(ast.Body2S(b2, diags))))
